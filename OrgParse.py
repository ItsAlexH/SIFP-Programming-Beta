...
async def update_events_by_id(bot, wks, program, calendar, event_ID, update_args = None):
    ...
    if(event0 is not None):
        ...
        if update_args.get("title") is not None: event0["title"] = update_args["title"]
        if update_args.get("date") is not None: event0["date"] = datetime.datetime.fromisoformat(update_args["date"]).date()
        if update_args.get("start_time") is not None: event0["start_time"] = datetime.datetime.strptime(update_args["start_time"], "%I:%M %p").time()
        if update_args.get("end_time") is not None: event0["end_time"] = datetime.datetime.strptime(update_args["end_time"], "%I:%M %p").time()
        ...
        event_date = datetime.datetime.fromisoformat(event0["date"]).date()
        start_time = datetime.datetime.fromisoformat(event0["start_time"]).time()
        end_time = datetime.datetime.fromisoformat(event0["end_time"]).time()

        start_time_date = eastern.localize(datetime.datetime.combine(event_date, start_time))
        end_time_date = eastern.localize(datetime.datetime.combine(event_date, end_time))
        
        week_number = int(event0["week"])
        print("Successfully Updated Internal Memory of Event")

        start_time_date = parse_times([event_date], [start_time_date])[0]
        end_time_date = parse_times([event_date], [end_time_date])[0]
        ...
        row_offset = 4
        if(ii != None):
            wks.get_worksheet(week_number+2).update_cell(ii+row_offset, 1, event0["date"])
            ...
            wks.get_worksheet(week_number+2).update_cell(ii+row_offset, 5, event0["start_time"])
            wks.get_worksheet(week_number+2).update_cell(ii+row_offset, 6, event0["end_time"])
